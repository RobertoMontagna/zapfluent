...
func (m *BePresentMatcher[T]) Match(actual any) (bool, error) {
	...
	if !ok {
		return false, fmt.Errorf("BePresent matcher expects an optional.Optional[%T]", *new(T))
	}
...
func (m *BeEmptyMatcher[T]) Match(actual any) (bool, error) {
	opt, ok := actual.(optional.Optional[T])
	if !ok {
		return false, fmt.Errorf("BeEmpty matcher expects an optional.Optional[%T]", *new(T))
	}
	return !opt.IsPresent(), nil
}
...
func (m *HaveValueMatcher[T]) Match(actual any) (bool, error) {
	opt, ok := actual.(optional.Optional[T])
	if !ok {
		return false, fmt.Errorf("HaveValue matcher expects an optional.Optional[%T]", *new(T))
	}
